...
"""
CotatQ v1.2.1 — compatibility-fixed external rival worker.

PATCH SCOPE ONLY:
- CotatQ frozen code is unchanged.
- Cases, seeds, timeouts, limits and verdict thresholds are unchanged.
- Fixes cotengra RandomGreedyOptimizer adapter for cotengra 0.8.2.
- Replaces quimb's auto-hq preset with an explicit non-parallel reusable
  cotengra hyper-optimizer, preventing nested process-pool failures on
  Windows/Python 3.14.

Every engine still runs in a fresh process.
"""
...
def build_cotengra_geometry(nodes):
    """
    Build the native cotengra geometry:

        inputs, output, size_dict

    cotengra 0.8.2 documents RandomGreedyOptimizer as:
        opt(inputs, output, size_dict)

    This adapter deliberately calls cotengra DIRECTLY and does not route the
    optimizer object through opt_einsum's PathOptimizer calling convention.
    """
    inputs = []
    size_dict = {}
    ...
    for node in nodes:
        ...
        if len(inds) != len(node.data.shape):
            raise ValueError("node labels/shape length mismatch")
        ...
        for ind, dim in zip(inds, node.data.shape):
            ...
            if old is not None and old != dim:
                raise ValueError(
                    f"inconsistent dimension for index {ind}: {old} vs {dim}"
                )
            size_dict[ind] = dim

    output = tuple()  # scalar amplitude
    return tuple(inputs), output, size_dict
...
def path_info_from_explicit_path(operands, path):
    """
    Get opt_einsum's largest-intermediate accounting for an already-found path.
    This does not perform a new path search. The small bookkeeping cost is counted
    in planning time for the rival.
    """
...
def bench_cotengra_rg128():
    """
    FIX #1:
    Call RandomGreedyOptimizer directly using the cotengra-native 3-argument API.
    """
    if IMPORT_ERROR:
        return {
            "ok": False,
    ...
    # no oe.contract_path(... optimize=optimizer)
    path = optimizer(inputs, output, size_dict)

    # Safety/accounting for the explicit path.
...
def bench_quimb_hq_serial():
    """
    FIX #2:
    Do NOT use optimize='auto-hq'.

    On Windows/Python 3.14 the preset produced BrokenProcessPool in the isolated
    worker. Instead quimb receives an explicit ReusableHyperOptimizer with
    parallel=False. The 2-second planning budget and hyper-optimization policy
    remain fixed and visible.

    quimb itself still builds the tree/expression and executes the raw arrays.
    """
    if IMPORT_ERROR:
        return {
            "ok": False,
    ...
    # Explicitly serial optimizer: no nested process pool.
